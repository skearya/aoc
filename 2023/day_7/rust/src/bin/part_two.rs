...
fn get_type<'a>((cards, bid): (&'a str, &'a str)) -> (Hand, &'a str, &'a str) {
    ...
    let j_cards = map.get(&'J').unwrap_or(&0);
    let largest_kind = map.iter().fold(
        0,
        |acc, (card, count)| {
            if *card != 'J' {
                acc.max(*count)
    ...
    (hand_type(*j_cards, largest_kind, len), cards, bid)
}

fn hand_type(j_cards: u16, largest_kind: u16, len: usize) -> Hand {
    if j_cards == 0 {
        return match largest_kind {
            5 => Hand::FiveOfAKind,
            4 => Hand::FourOfAKind,
            3 => {
                if len == 2 {
    ...
    if j_cards == 5 - largest_kind {
        Hand::FiveOfAKind
    } else if j_cards == 5 - (largest_kind + 1) {
        Hand::FourOfAKind
    } else if j_cards == 5 - (largest_kind + 2) {
        if len == 3 {
            Hand::FullHouse
        } else {
            Hand::ThreeOfAKind
        }
    } else if j_cards == 5 - (largest_kind + 3) {
        if len == 3 {
            Hand::TwoPair
        } else {
            Hand::OnePair
