...
fn main() {
    ...
    let starting_dir = starting_dir(&points, &starting_coords);

    let path = traverse(
        &points,
        &starting_dir.apply_to_coords(starting_coords),
        starting_dir,
    );
...
fn traverse(
    ...
    loop {
        ...
        prev_dir = exclude(&possible_dirs, !prev_dir).unwrap();
        (row, col) = prev_dir.apply_to_coords((row, col));
    }
